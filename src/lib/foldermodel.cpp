 ...
 *   Copyright (C) 2021 by Reven Martin <revenmartin@gmail.com>            *
 *                                                                         *
...
Q_LOGGING_CATEGORY(FOLDERMODEL, "cutefish.desktop.folder.foldermodel")
    ...
    , m_complete(false)
{
    ...
    qmlRegisterAnonymousType<KIO::DropJob>("Cutefish.FileManager", 1);
...
//        if (action) {
//            action->setText(m_usedByContainment ? tr("&Refresh Desktop") : tr("&Refresh View"));
...
void FolderModel::createActions()
{
    ...
    connect(undo, &QAction::triggered, manager, &KIO::FileUndoManager::undo);

    connect(manager, SIGNAL(undoAvailable(bool)), undo, SLOT(setEnabled(bool)));
    // revenmartin: not needed for now.
    ...
    // QAction *refresh = new QAction(tr("&Refresh View"), this);
    ...
    connect(newFolderAction, &QAction::triggered, this, &FolderModel::createFolder);

    QAction *newDocAction = new QAction(tr("New Documents"));
    connect(newDocAction, &QAction::triggered, this, [=] {
        m_newMenu->setPopupFiles(QList<QUrl>() << m_dirModel->dirLister()->url());
        m_newMenu->createFile();
    });
    ...
    QAction *restoreFromTrash = new QAction(tr("Restore from trash"), this);
...
void FolderModel::updateActions()
{
    ...
    if (QAction *newFolder = m_actionCollection.action(QStringLiteral("new_folder"))) {
        newFolder->setVisible(!isTrash);
    }

    if (QAction *newDocuments = m_actionCollection.action(QStringLiteral("new_documents"))) {
        newDocuments->setVisible(!isTrash);
    }
...
void FolderModel::openContextMenu(QQuickItem *visualParent, Qt::KeyboardModifiers modifiers)
{
    ...
    if (indexes.isEmpty()) {
        ...
        if (KFileItem(url()).isWritable())
            menu->addAction(m_actionCollection.action(QStringLiteral("new_folder")));

        if (KFileItem(url()).isWritable())
            menu->addAction(m_actionCollection.action(QStringLiteral("new_documents")));
        ...
        if (m_isDesktopView) {
            QAction *setWallpaper = new QAction(tr("Change Wallpaper"), this);
            connect(setWallpaper, &QAction::triggered, this, [=] { openSettings("wallpaper"); });
            menu->addSeparator();
        ...
        deleteAction->setVisible(!trashAction->isVisible());

        menu->addSeparator();

        // Set as Wallpaper
        if (indexes.count() == 1 && items.first().mimetype().startsWith("image/")) {
            QAction *setAswallpaper = new QAction(tr("Set as Wallpaper"), this);
            connect(setAswallpaper, &QAction::triggered, this, &FolderModel::setAsWallpaper);
...
void FolderModel::setAsWallpaper()
{
    if (!m_selectionModel)
        return;
...
void FolderModel::openSettings(const QString &itemName)
{
    QProcess process;
    process.startDetached("cutefish-settings", QStringList() << "-m" << itemName);
}

bool FolderModel::desktopView() const
{
    return m_isDesktopView;
}

void FolderModel::setDesktopView(bool value)
{
    if (m_isDesktopView != value) {
        m_isDesktopView = value;
